...
type View = "dashboard" | "cast" | "abjad" | "question" | "celestial";

function FigureGlyph({ pattern }: { pattern: readonly number[] }) {
  return (
    <div className="glyph" aria-label={pattern.join("-")}>
...
export default function App() {
  ...
  const planetary = useMemo(() => {
    try {
      return currentPlanetaryHour(moment, latitude, longitude);
    } catch {
      return null;
    }
    ...
    <div className="shell">
      <aside>
        <div className="brand">
          ...
          <div>
            <strong>ALLAMA ABSOLUTE</strong>
            <small>Sovereign Symbolic Intelligence Engine</small>
          </div>
        </div>
      ...
      <main>
        <header>
          <div>
            ...
            <h1>{view === "dashboard" ? "Command Center" : view.toUpperCase()}</h1>
          </div>
          ...
          <section className="grid">
            <article className="hero">
              ...
              <div className="hero-actions">
                <button onClick={() => setView("cast")}>Open Shield</button>
                <button className="secondary" onClick={() => setView("question")}>Analyze Question</button>
              </div>
            </article>
        ...
        {view === "cast" && (
          <section>
            <div className="toolbar">
              ...
              <button
                className="secondary"
                onClick={() => downloadText("allama-shield.json", toJson(shield), "application/json")}
              >
                Export Shield JSON
              </button>
            </div>
                  ...
                  <select value={id} onChange={(event) => updateMother(index, event.target.value)}>
                    {FIGURES.map((figure) => (
                      <option key={figure.id} value={figure.id}>
                        {figure.latin} — {figure.pattern.join("")}
                      </option>
                    ))}
                  </select>
                </label>
            ...
            <div className="validation">
              {validateShield(shield).length
                ? validateShield(shield).join("; ")
                : "✓ Structural validation passed, including Judge parity."}
            </div>
          </section>
        )}
        ...
        {view === "abjad" && (
          <section className="panel">
            <label>
              Arabic text
              <input dir="rtl" value={abjadText} onChange={(event) => setAbjadText(event.target.value)} />
            </label>
            ...
            <table>
              ...
              <tbody>
                {abjad.steps.map((step, index) => (
                  <tr key={index}><td dir="rtl">{step.letter}</td><td>{step.value}</td></tr>
                ))}
              </tbody>
            </table>
          </section>
